...
- `relay.blindwire.tech` A record pointing to the VPS
...
1. Refuses to continue until `relay.blindwire.tech` resolves.
...
curl --fail --silent --show-error --https-only \
  https://relay.blindwire.tech/healthz
...
sudo deploy/scripts/print-spki-pin.sh \
  /etc/letsencrypt/live/relay.blindwire.tech/cert.pem
```
